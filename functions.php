...
function agregar_tailwind() {
    wp_enqueue_style('tailwind', get_template_directory_uri() . '/src/output.css?v=' . time(), array(), null);
}
add_action('wp_enqueue_scripts', 'agregar_tailwind');
...
function enqueue_parent_styles() {
    ...
    wp_enqueue_style('tailwind-style', get_stylesheet_directory_uri() . '/tailwind.css');
}
...
function custom_post_type() {
    ...
    $args = array(
        ...
        'show_in_rest'       => true, // Soporta el editor Gutenberg
    );
...
function include_custom_posts_in_main_query($query) {
    if (!is_admin() && $query->is_main_query() && is_home()) {
        $query->set('post_type', array('post', 'custom_posts')); // Muestra posts normales y Custom Posts
    }
}
add_action('pre_get_posts', 'include_custom_posts_in_main_query');
